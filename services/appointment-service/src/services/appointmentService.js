...
const createAppointment = async (data) => {
  ...
  return Appointment.findById(appointment._id).populate("patient doctor", "fullName email phone role");
};
...
const getAppointments = async (filters = {}) => {
  ...
  const appointments = await Appointment.find(query)
    .populate("patient doctor", "fullName email phone role")
    .sort(sort)
    .skip(skip)
...
const updateAppointment = async (id, data) => {
  ...
  return appointment.populate("patient doctor", "fullName email phone role");
};
...
const transitionStatus = async (id, newStatus) => {
  const appointment = await Appointment.findById(id).populate("patient doctor", "fullName email phone role");
  if (!appointment) {
    const error = new Error("Appointment not found");
    error.statusCode = 404;
    throw error;
